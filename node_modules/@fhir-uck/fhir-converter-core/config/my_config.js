...
module.exports.beforeProcess = (data) => {
    // 計算 BMI，根據 csv 欄位 Height(cm) 和 Weight(kg)
    if (data['Height(cm)'] && data['Weight(kg)']) {
        let heightM = parseFloat(data['Height(cm)']) / 100;
        let weightKg = parseFloat(data['Weight(kg)']);
        data.bmi = +(weightKg / (heightM * heightM)).toFixed(2);
        // 也可同步設置 heightValue/weightValue 以供 Observation 使用
        data.heightValue = parseFloat(data['Height(cm)']);
        data.weightValue = parseFloat(data['Weight(kg)']);
    }
    return data;
}
